...
def calc_total_energy(spec: Dict):
    ...
    E_W, E_E_CG_gen_d_t, E_E_TU_aux_d_t, E_G_CG_ded, e_BB_ave, Q_CG_h, E_E_W_d_t, E_G_W_d, E_K_W_d_t, E_G_CG_d_t, E_K_CG_d_t \
            = calc_E_W(spec_MR, spec_OR, mode_MR, mode_OR, spec_HS, L_T_H_d_t_i, L_dash_H_R_d_t_i, n_p, heating_flag_d, spec['A_A'], spec['region'], spec['sol_region'], spec_HW, spec['SHC'], spec['CG'], L_HWH,
...
# 1 年当たりの給湯設備（コージェネレーション設備を含む）の設計一次エネルギー消費量
def calc_E_W(spec_MR, spec_OR, mode_MR, mode_OR, spec_HS, L_T_H_d_t_i, L_dash_H_R_d_t_i, n_p, heating_flag_d, A_A, region, sol_region, HW, SHC, CG, L_HWH, H_A=None, H_HS=None, C_A=None, C_MR=None,
            C_OR=None,
