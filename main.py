...
def calc_total_energy(spec: Dict):
    ...
    E_S = section2_2.calc_E_S(spec['region'], spec['sol_region'], spec['PV'], spec['CG'], E_E_dmd_d_t, E_E_CG_gen_d_t,
    ...
    print('計算値: ' + str(E_T))
    print('暖房: ' + str( round(E_H/1000)))
    print('冷房: ' + str( round(E_C/1000)))
    print('換気: ' + str( round(E_V/1000)))
    print('照明: ' + str( round(E_L/1000)))
    print('給湯: ' + str( round(E_W/1000)))
    print('自家消費分: ' + str( round(E_S/1000)))
    print('その他: ' + str( round(E_M/1000)))
...
# 1 年当たりの給湯設備（コージェネレーション設備を含む）の設計一次エネルギー消費量
def calc_E_W(A_A, region, sol_region, HW, SHC, CG, L_HWH, H_A=None, H_MR=None, H_OR=None, H_HS=None, C_A=None, C_MR=None,
            C_OR=None,
    ...
    # 1日当たりの給湯設備の消費電力量
    E_E_W_d_t = section2_2.calc_E_E_W_d_t(n_p, L_HWH, heating_flag_d, region, sol_region, spec_HW, SHC)

    # 1日当たりの給湯設備のガス消費量 (MJ/d)
    E_G_W_d = section2_2.calc_E_G_W_d_t(n_p, L_HWH, heating_flag_d, A_A, region, sol_region, HW, SHC)

    # 1日当たりのコージェネレーション設備のガス消費量
    E_G_CG_d_t, *args = section2_2.calc_E_CG_d_t(A_A, region, sol_region, HW, SHC, CG, H_A, H_MR, H_OR, H_HS, C_A, C_MR,
                           C_OR,
                           V, L, A_MR, A_OR, A_env, Q, mu_H, mu_C, NV_MR, NV_OR, TS,
                           r_A_ufvnt, HEX, underfloor_insulation, mode_H, mode_C)

    # 1日当たりの給湯設備の灯油消費量 (MJ/d)
    E_K_W_d_t = section2_2.calc_E_K_W_d_t(n_p, L_HWH, heating_flag_d, A_A, region, sol_region, HW, SHC)
    ...
    if HW['hw_type'] != 'コージェネレーションを使用する':
        ...
        E_G_CG_d_t, E_E_CG_gen_d_t, E_E_CG_h_d_t, E_E_TU_aux_d_t, E_E_CG_h_d_t, E_G_CG_ded, e_BB_ave, Q_CG_h = \
            section2_2.calc_E_CG_d_t(A_A, region, sol_region, HW, SHC, CG, H_A, H_MR, H_OR, H_HS, C_A, C_MR, C_OR,
                          V, L, A_MR, A_OR, A_env, Q, mu_H, mu_C, NV_MR, NV_OR, TS,
                                             r_A_ufvnt, HEX, underfloor_insulation, mode_H, mode_C)
